...
- **No "spectator" → "Spectator" capitalization in code paths or
  ...
  `config.TOOL_TREE_RELPATH`) — there it represents the project tree
  the way humans see it browsing `ls $workdir/`. Lowercase still
  applies to the wrapper filename, Python module name, env vars, and
